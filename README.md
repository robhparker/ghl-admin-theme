...
(function () {
  ...
  s.src = 'https://cdn.jsdelivr.net/gh/robhparker/admin-theme@v0.1.0/src/ghl-customizer.js';
  s.setAttribute('data-config', 'https://cdn.jsdelivr.net/gh/robhparker/admin-theme@v0.1.0/config/agency-config.json');
  s.defer = true;
...
```html
<script src="https://cdn.jsdelivr.net/gh/robhparker/admin-theme@v0.1.0/src/ghl-customizer.js"
        data-config="https://cdn.jsdelivr.net/gh/robhparker/admin-theme@v0.1.0/config/agency-config.json"
        defer></script>
```
...
"iDPNGKoFsjvf9wUCrk3V": {
  "name": "Dummy Clinic",
  "logoUrl": "https://cdn.jsdelivr.net/gh/robhparker/admin-theme@v0.1.0/test/fixtures/logos/loc-a.svg",
...
Verified live on 2026-09-24: the sidebar container `#sidebar-v2` and its logo `#sidebar-v2 img.agency-logo`, the header `.hl_header` with its `.hl_header--controls`, the contact panel `#record-details-lhs` whose name row (the parent of `#delete-contact-trigger`) is the contact button mount, and the email and phone fields with element ids `contact.email` and `contact.phone`. The active sidebar navigation item is a candidate list (`selectors.sidebarNavActive`) that is confirmed or corrected during the live install check.
